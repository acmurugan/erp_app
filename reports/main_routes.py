...
def convert_db_param_to_form_param_universal(param, report_type):
    ...
    visible = param.get('visible', True)  # PRESERVE VISIBILITY SETTING
...
def parse_rpt_params_universal(rpt_params_clob, report_type, filter_hidden=True):
    """Universal RPT_PARAMS parser that handles ALL formats"""
    
    import traceback
    print(f"*** PARSE_RPT_PARAMS_UNIVERSAL CALLED for {report_type}, filter_hidden={filter_hidden} ***")
    print(f"*** Call stack: {traceback.format_stack()[-2].strip()} ***")
    
    if not rpt_params_clob:
        error_msg = f"RPT_PARAMS is NULL/empty for report type {report_type}"
        print(f"ERROR: {error_msg}")
        raise ValueError(f"Configuration Error: {error_msg}. Please check RPT_PARAMS in RPT_REPORT_MASTER table.")
    ...
    try:
        ...
        if not rpt_params_content:
            error_msg = f"RPT_PARAMS content is empty after reading CLOB for report type {report_type}"
            print(f"ERROR: {error_msg}")
            raise ValueError(f"Configuration Error: {error_msg}. CLOB column exists but contains no data.")
        ...
        if rpt_params_content.startswith('{') or rpt_params_content.startswith('['):
            try:
                ...
                # Define session variables that should NOT appear in the form
                # These are automatically provided from the user session
                session_variables = {
                    'comp_code', 'company_code', 'user_id', 'user_name', 'branch_code',
                    'lang_code', 'language_code', 'm_comp_code', 'm_user_id', 'm_lang_code'
                }
                ...
                for param in param_list:
                    ...
                    if isinstance(converted_param, list):
                        ...
                        for sub_param in converted_param:
                            # Filter out session variables
                            field_name = sub_param.get('field', '').lower()
                            # Check visibility (default to true if not specified)
                            is_visible = sub_param.get('visible', True)
                            
                            if field_name not in session_variables and (is_visible or not filter_hidden):
                                parameters.append(sub_param)
                                print(f"SUCCESS Added parameter: {sub_param['field']} ({sub_param['type']})")
                            elif field_name in session_variables:
                                print(f"FILTERED Session variable excluded from form: {sub_param['field']}")
                            elif not is_visible and filter_hidden:
                                print(f"FILTERED Hidden parameter excluded from form: {sub_param['field']} (visible=false)")
                    else:
                        # Filter out session variables and hidden parameters
                        field_name = converted_param.get('field', '').lower()
                        # Check visibility (default to true if not specified)
                        is_visible = converted_param.get('visible', True)
                        
                        if field_name not in session_variables and (is_visible or not filter_hidden):
                            parameters.append(converted_param)
                            print(f"SUCCESS Added parameter: {converted_param['field']} ({converted_param['type']})")
                        elif field_name in session_variables:
                            print(f"FILTERED Session variable excluded from form: {converted_param['field']}")
                        elif not is_visible and filter_hidden:
                            print(f"FILTERED Hidden parameter excluded from form: {converted_param['field']} (visible=false)")
        ...
        error_msg = f"Failed to parse RPT_PARAMS for report type {report_type}: {str(e)}"
        print(f"ERROR: {error_msg}")
        raise ValueError(f"Configuration Error: {error_msg}. Check JSON syntax in RPT_PARAMS column.")
...
def validate_and_fix_parameters_globally(params, report_id):
    ...
    # Handle date parameters based on EXACT RPT_PARAMS configuration
    # Don't add generic date parameters - let RPT_PARAMS processing handle the exact names
    print(f"STEP Date parameter processing: Found {len([k for k in fixed_params.keys() if 'date' in k.lower() or 'dt' in k.lower()])} date-related parameters")
...
def report_parameter_form(form_id, menu_id):
    ...
    print(f"CHART Loading dynamic form - Form ID: {form_id}, Menu ID: {menu_id}")
    ...
    try:
        ...
        # If no parameters parsed from RPT_PARAMS, show error
        if not parameters:
            error_msg = f"No valid parameters found in RPT_PARAMS for {report_config['type']} report"
            print(f"ERROR: {error_msg}")
            return f'''
            <div class="alert alert-danger">
                <h4>Configuration Error: {form_id}</h4>
                <p><strong>Error:</strong> {error_msg}</p>
                <p><strong>Solution:</strong> Check RPT_PARAMS JSON configuration in RPT_REPORT_MASTER table</p>
                <p><strong>Report Type:</strong> {report_config['type']}</p>
            </div>
            '''
...
# Fallback system removed - now shows proper error messages instead of misleading defaults
...
def generate_report(form_id, menu_id):
    ...
    try:
        ...
        print(f"DEBUG About to execute report with form_id={form_id}, params keys: {list(params.keys())}")
        print(f"DEBUG Full params being sent to executor:")
        ...
        # Debug: Check if column filtering was applied
        if 'columns' in report_data and 'all_columns' in report_data:
            print(f"DEBUG Column filtering detected:")
            print(f"   Display columns: {len(report_data['columns'])} - {report_data['columns']}")
            print(f"   All columns: {len(report_data['all_columns'])} - {report_data['all_columns']}")
        elif 'columns' in report_data:
            print(f"DEBUG No column filtering applied - columns: {len(report_data['columns'])}")
        
        # Handle different output formats
        if output_format == 'excel':
            print("CHART Generating Excel report...")
        ...
        print(f"DEBUG TEMPLATE data.get('column_headers'): {report_data.get('column_headers', 'NOT_FOUND')}")
        print(f"DEBUG TEMPLATE data.columns[:5]: {report_data.get('columns', [])[:5]}")
...
def build_dynamic_parameters(form_data, session, report_config):
    ...
    print(f"*** BUILD_DYNAMIC_PARAMETERS CALLED for {report_type} report: {form_id} ***")
    print(f"GLOBAL GLOBAL PARAMETER BUILDER for {report_type} report: {form_id}")
    
    # Base parameters for all reports
    params = {
        'user_id': session.get('user_id', 'SYSTEM'),
        'comp_code': session.get('comp_code', 'TTM'),  # Add company code from session
        'report_id': form_id,
    ...
    # Add hidden parameters from RPT_PARAMS configuration
    try:
        # Get RPT_PARAMS and find hidden parameters - FIXED: use 'params' not 'rpt_params'
        ...
        if rpt_params:
            ...
            if rpt_config and 'parameters' in rpt_config:
                print(f"STEP Processing {len(rpt_config['parameters'])} parameters from RPT_PARAMS configuration")
                for param in rpt_config['parameters']:
                    field_name = param.get('field')
                    is_visible = param.get('visible', True)
                    default_value = param.get('default')
                    
                    print(f"   CONFIG Parameter: {field_name} (visible: {is_visible}, default: '{default_value}')")
                    
                    # If parameter is NOT in form data (either visible or hidden), add default value
                    if field_name and field_name not in params and default_value:
                        # Add parameter in both original case and lowercase for Oracle compatibility
                        params[field_name] = default_value
                        params[field_name.lower()] = default_value
                        if is_visible:
                            print(f"ADDED Added missing visible parameter: {field_name} = '{default_value}' (both cases)")
                        else:
                            print(f"HIDDEN Added hidden parameter: {field_name} = '{default_value}' (both cases)")
                    
                    # Also ensure case variations exist for form matching
                    if field_name and field_name in params:
                        # Ensure both cases exist for Oracle compatibility
                        if field_name.upper() not in params:
                            params[field_name.upper()] = params[field_name]
                        if field_name.lower() not in params:
                            params[field_name.lower()] = params[field_name]
                        print(f"CASE Added case variants for existing parameter: {field_name}")
    except Exception as e:
        print(f"WARNING Failed to process hidden parameters: {e}")
    
    # Use global validation and fixing
    final_params = validate_and_fix_parameters_globally(params, form_id)
    
    # CRITICAL FIX: Re-add hidden parameters at the very end to ensure they're not lost
    try:
        rpt_params = report_config.get('params')
        if rpt_params:
            if isinstance(rpt_params, str):
            ...
            if rpt_config and 'parameters' in rpt_config:
                for param in rpt_config['parameters']:
                    ...
                    # FORCE ADD hidden parameters even if they were somehow removed
                    if not is_visible and field_name and default_value:
                        final_params[field_name] = default_value
                        final_params[field_name.lower()] = default_value
                        print(f"FINAL FORCE Added hidden parameter: {field_name} = '{default_value}' (both cases)")
    except Exception as e:
        print(f"WARNING Failed to force-add hidden parameters: {e}")
    ...
    try:
        ...
        if report_config:
            ...
            if report_config.get('params'):
                ...
                debug_info['error'] = 'RPT_PARAMS is empty or NULL - no fallback system available'
                debug_info['solution'] = 'Check and update RPT_PARAMS JSON in RPT_REPORT_MASTER table'
        else:
            ...
            <div class="container mt-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <h5>CHART Supported RPT_PARAMS Formats:</h5>
                        <ul>
...
# Add this to the END of your reports\main_routes.py file
# (after all your existing routes)

@reports_bp.route('/converter')
def forms6i_converter():
    """Forms 6i to Python Converter Tool"""
    if 'user_id' not in session:
        return redirect('/login')
    
    return render_template('reports/forms6i_converter.html')

@reports_bp.route('/converter/analyze', methods=['POST'])
def analyze_forms6i_code():
    """Analyze Forms 6i code and return conversion results"""
    if 'user_id' not in session:
    ...
    try:
        ...
        forms6i_code = data.get('forms6i_code', '').strip()
        
        if not all([report_id, report_name, forms6i_code]):
            return jsonify({'error': 'Missing required fields'}), 400
        
        # Simple analysis (you can enhance this later)
        analysis = {
            'cursors': len(re.findall(r'CURSOR\s+\w+', forms6i_code, re.IGNORECASE)),
            'parameters': len(re.findall(r':REP_VALUE_\d+|:GLOBAL\.\w+', forms6i_code, re.IGNORECASE)),
            'complexity': 'COMPLEX' if 'SET_GROUP_' in forms6i_code else 'SIMPLE',
            'report_type': 'CLASS' if 'SET_GROUP_' in forms6i_code else 'SQL'
        }
        
        return jsonify({
            'success': True,
            'analysis': analysis,
            'message': f'Analyzed successfully: {analysis["cursors"]} cursors, {analysis["parameters"]} parameters'
        })
        
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    
@reports_bp.route('/test/update-display-columns/<report_id>')
def test_update_display_columns(report_id):
    """Test route to add display column configuration"""
    if 'user_id' not in session:
        return redirect('/login')
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if current_params:
                ...
                # Add display columns configuration - show only key columns
                if report_id == 'FIN001':
                    ...
                    # Generic test - show first 5 columns
                    params_config['display_columns'] = []  # Will be set based on actual columns
                ...
                cursor.execute("""
                    ...
                    WHERE RPT_ID = :report_id
                """, {'new_params': new_params, 'report_id': report_id})
                
                conn.commit()
                
                return f"""
                <h3>SUCCESS: Updated {report_id} with display column configuration</h3>
                <p><strong>Display columns:</strong> 6 key business columns</p>
                <p><strong>Column headers:</strong> User-friendly names</p>
                <hr>
                <p><strong>Test the functionality:</strong></p>
                <p><a href="/reports/{report_id}/T030101" target="_blank">Run {report_id} Report</a></p>
                <p><strong>Expected result:</strong> Report should show only 6 columns instead of all 25</p>
                <hr>
                <p><a href="/reports/test/reset-display-columns/{report_id}">Reset to show all columns</a></p>
                """
            else:
                return f"No RPT_PARAMS found for {report_id}"
                
    except Exception as e:
        return f"Error: {e}"

@reports_bp.route('/test/reset-display-columns/<report_id>')
def test_reset_display_columns(report_id):
    """Test route to reset display column configuration"""
    if 'user_id' not in session:
        return redirect('/login')
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if current_params:
                ...
                return f"""
                <h3>SUCCESS: Reset {report_id} to show all columns</h3>
                <p><a href="/reports/{report_id}/T030101" target="_blank">Run {report_id} Report</a></p>
                <p><strong>Expected result:</strong> Report should show all columns again</p>
                <hr>
                <p><a href="/reports/test/update-display-columns/{report_id}">Configure display columns again</a></p>
                """
            else:
                return f"No RPT_PARAMS found for {report_id}"
...
print("GLOBAL UNIVERSAL REPORT SYSTEM LOADED!")
print("Features:")
...
print("This system will handle ANY current and future report!")
print("Updated functions:")
print("   - parse_rpt_params_dynamic -> parse_rpt_params_universal")
print("   - convert_db_param_to_form_param -> convert_db_param_to_form_param_universal") 
print("   - build_dynamic_parameters -> global parameter builder")
print("   - validate_and_fix_parameters_globally -> global validation")