...
def process_menu_hierarchy(menu_results):
    ...
    for menu_data in menu_results:
        # Handle different tuple lengths (6, 7, or 8 elements)
        if len(menu_data) == 8:
            menu_id, parent_id, action_type, scr_name, seq_no, button_1, menu_action, menu_level = menu_data
        elif len(menu_data) == 7:
            menu_id, parent_id, action_type, scr_name, seq_no, button_1, menu_action = menu_data
            menu_level = 1  # Default level
        else:
...
def get_user_menus(username):
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Simplified hierarchical query using CONNECT BY - more efficient
            menu_scr_select = f"INITCAP(DECODE('{lang_code}', 'ENG', mm.MENU_OPTION_DESC, mm.MENU_SCR_NAME_BL)) AS MENU_SCR_NAME"
            menus_query = f"""
                SELECT mm.MENU_ID,
                       mm.MENU_PARENT_ID,
                       mm.MENU_ACTION_TYPE,
                ...
                WHERE  mu.USER_ID = :1
                START WITH mm.MENU_PARENT_ID = '*'
                CONNECT BY PRIOR mm.MENU_ID = mm.MENU_PARENT_ID
                   AND PRIOR mu.USER_ID = mu.USER_ID
                ORDER SIBLINGS BY mm.MENU_DISP_SEQ_NO, mm.MENU_SEQ_NO
            """
...
def get_report_menus(username):
    ...
    try:
       ...
       LEVEL AS menu_level
FROM   MENU_USER mu
JOIN   MENU_USER_MENUS umm
       ON mu.USER_GROUP_ID = umm.UM_GROUP_ID
JOIN   MENU_MENUS mm
       ON mm.MENU_ID = umm.UM_MENU_ID
WHERE  mu.USER_ID = :1
AND  (mm.MENU_BUTTON_1 LIKE '/reports/%' OR mm.MENU_ACTION_TYPE = 'R')
START WITH mm.MENU_PARENT_ID = '*'
CONNECT BY PRIOR mm.MENU_ID = mm.MENU_PARENT_ID
           AND PRIOR mu.USER_ID = mu.USER_ID
ORDER SIBLINGS BY mm.MENU_SEQ_NO        
    """
